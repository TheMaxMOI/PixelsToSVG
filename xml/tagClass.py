...
def isAttributeList(lst):
    return (
        type(lst) == list and 
        len(lst) != 0 and 
        type(lst[0]) == tuple and 
        len(lst[0]) == 2 and 
        type(lst[0][0]) == type(lst[0][1]) == str 
    )
...
class Tag:
    def __init__(self, name: str, attributes: list[tuple[str,str]] | None = None, isEmpty = False):
        self.name = name
    ...
    def setData(self, data: list[str|Tag]):
        if self.data == None:
            raise TypeError(f"Tag: setData: The tag name {self.name} is meant to be empty!")
