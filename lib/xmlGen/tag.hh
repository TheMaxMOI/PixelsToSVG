...
concept Callable = requires(TagFunc f, Tag& tag) { f(tag); };
...
protected:
    ...
    getAttributeValue_(const std::string& attrName) const;

public:
    Tag(const std::string& name, const std::vector<attr_t>& attributes = {},
        bool isEmpty = false);
