...
Appearance::Appearance(const std::string& coloring_key,
                       const std::string& color, double opacity)
    : coloring_key_{ coloring_key }
    , color_{ color }
    , opacity_{ opacity }
{}
...
Outline::Outline(const std::string& stroke, double width, double opacity)
    : Appearance{ "stroke", stroke, opacity }
    , width_{ width }
{}
...
{
    const auto& color =
        rgb(randint(MAX_UINT8), randint(MAX_UINT8), randint(MAX_UINT8));
